...
The translation process works on Windows, Linux and macOS. It is driven by `tasks.py`,
which needs the GNU gettext tools (`xgettext` and `msgfmt`) on your `PATH`:

* Linux: install the `gettext` package with your distribution's package manager
...
First you need to create template files (*.pot). Got you covered here, just execute
`python tasks.py translation-templates` (or `make translation-templates`) in the repos main directory. 
...
you can just open them with a text editor of your choice and start translating.
   ...
   3) Now you need to compile to *.po files to *.mo files. Again we've got you covered, just execute
   `python tasks.py compile-translations` (or `make compile-translations`) in the repos main directory.
...
On Windows PowerShell the equivalent is `$env:LANGUAGE="LANGISOCODE"; python tasks.py dev` from the repo root. 
